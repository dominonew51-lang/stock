...
function HoldingsTable({ holdings, allCount, filter, setFilter, quotes, quoteErrors, onLookup, onSaveAll, onDelete }: { holdings: Holding[]; allCount: number; filter: "全部" | AssetBucket; setFilter: (value: "全部" | AssetBucket) => void; quotes: Record<string, MarketQuote>; quoteErrors: Record<string, string>; onLookup:(symbol:string)=>Promise<MarketQuote | null>; onSaveAll:(edits:{item:Holding;originalSymbol:string}[])=>void; onDelete:(symbol:string)=>void }) {
  ...
  const editableRow = (draft: Holding, key: string, isNew = false) => {
    ...
    return <div className={`holding-row row-editing ${isNew ? "new-holding-row" : ""}`} key={key}>
  ...
  return <section className={`panel holdings-panel ${editing ? "holdings-edit-mode" : ""}`}>
    <div className="panel-head holdings-head">
      ...
      <div className="holdings-actions">{editing ? <span className="edit-mode-actions"><button type="button" onClick={cancelEdit}>取消</button><button type="button" onClick={()=>void saveAll()}>保存全部</button></span> : <><span className="quote-status"><i className="status-dot" />行情按市场时段更新 · 已更新 {Object.keys(quotes).length} 项</span><select className="bucket-filter" value={filter} onChange={(event)=>setFilter(event.target.value as "全部" | AssetBucket)} aria-label="按资产分类筛选"><option value="全部">全部分类</option>{assetBuckets.map((item)=><option key={item}>{item}</option>)}</select><span className="sort-controls"><select value={sortKey} onChange={(event)=>setSortKey(event.target.value as typeof sortKey)} aria-label="持仓排序方式"><option value="default">默认排序</option><option value="return">按收益率</option><option value="profit">按绝对收益</option><option value="value">按持仓市值</option></select><button type="button" onClick={()=>setSortDirection((current)=>current === "desc" ? "asc" : "desc")} disabled={sortKey === "default"} aria-label="切换排序方向">{sortDirection === "desc" ? "↓" : "↑"}</button></span><button type="button" className="portfolio-edit-toggle" onClick={beginEdit}>编辑持仓</button></>}</div>
    </div>
    ...
    <div className="holding-table">
      <div className="holding-row holding-header">{editing ? <><span>资产信息</span><span>持仓数据</span></> : <><span>资产</span><span>持仓（市值）</span><span>收益率</span><span>股价</span></>}</div>
